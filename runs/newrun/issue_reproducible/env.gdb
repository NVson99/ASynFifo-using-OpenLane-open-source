set env CLOCK_WIRE_RC_LAYER met5
set env CURRENT_DEF ./results/floorplan/AsynFifo.def
set env CURRENT_NETLIST ./results/synthesis/AsynFifo.v
set env CURRENT_ODB ./results/floorplan/AsynFifo.odb
set env CURRENT_POWERED_NETLIST 0
set env CURRENT_SDC ./tmp/floorplan/3-initial_fp.sdc
set env DATA_WIRE_RC_LAYER met2
set env DESIGN_NAME AsynFifo
set env FP_PDN_ENABLE_GLOBAL_CONNECTIONS 1
set env FP_PDN_ENABLE_MACROS_GRID 1
set env GND_NET vssd1
set env GPL_CELL_PADDING 0
set env GRT_ADJUSTMENT 0.3
set env GRT_LAYER_ADJUSTMENTS 0.99,0,0,0,0,0
set env GRT_MACRO_EXTENSION 0
set env LIB_TYPICAL pdk/sky130A/libs.ref/sky130_fd_sc_hd/lib/sky130_fd_sc_hd__tt_025C_1v80.lib
set env MAX_FANOUT_CONSTRAINT 18
set env MAX_TRANSITION_CONSTRAINT 0.5
set env MERGED_LEF ./tmp/merged.nom.lef
set env OUTPUT_CAP_LOAD 33.442
set env PACKAGED_SCRIPT_0 openlane/scripts/openroad/gpl.tcl
set env PACKAGED_SCRIPT_1 openlane/scripts/openroad/common/io.tcl
set env PACKAGED_SCRIPT_2 openlane/scripts/openroad/common/set_rc.tcl
set env PACKAGED_SCRIPT_3 openlane/scripts/openroad/common/set_routing_layers.tcl
set env PACKAGED_SCRIPT_4 openlane/scripts/openroad/common/set_layer_adjustments.tcl
set env PACKAGED_SCRIPT_5 ./tmp/floorplan/3-initial_fp.sdc
set env PACKAGED_SCRIPT_6 openlane/scripts/openroad/common/set_global_connections.tcl
set env PL_BASIC_PLACEMENT 0
set env PL_ROUTABILITY_DRIVEN 1
set env PL_SKIP_INITIAL_PLACEMENT 0
set env PL_TARGET_DENSITY 0.50
set env PL_TIME_DRIVEN 1
set env PL_WIRELENGTH_COEF 0.25
set env PROCESS_CORNER nom
set env RT_CLOCK_MIN_LAYER met3
set env RT_MAX_LAYER met5
set env RT_MIN_LAYER met1
set env SAVE_DEF ./tmp/placement/7-global.def
set env SAVE_NETLIST ./out.v
set env SAVE_ODB ./tmp/placement/7-global.odb
set env SAVE_POWERED_NETLIST ./tmp/placement/7-global.pnl.v
set env SCRIPTS_DIR openlane/scripts
set env STA_PRE_CTS 1
set env STD_CELL_GROUND_PINS VGND VNB
set env STD_CELL_POWER_PINS VPWR VPB
set env TECH_METAL_LAYERS li1 met1 met2 met3 met4 met5
set env VDD_NET vccd1