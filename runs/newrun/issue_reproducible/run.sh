#!/bin/sh
dir=$(cd -P -- "$(dirname -- "$0")" && pwd -P)
cd $dir;
export CLOCK_WIRE_RC_LAYER='met5';
export CURRENT_DEF='./results/floorplan/AsynFifo.def';
export CURRENT_NETLIST='./results/synthesis/AsynFifo.v';
export CURRENT_ODB='./results/floorplan/AsynFifo.odb';
export CURRENT_POWERED_NETLIST='0';
export CURRENT_SDC='./tmp/floorplan/3-initial_fp.sdc';
export DATA_WIRE_RC_LAYER='met2';
export DESIGN_NAME='AsynFifo';
export FP_PDN_ENABLE_GLOBAL_CONNECTIONS='1';
export FP_PDN_ENABLE_MACROS_GRID='1';
export GND_NET='vssd1';
export GPL_CELL_PADDING='0';
export GRT_ADJUSTMENT='0.3';
export GRT_LAYER_ADJUSTMENTS='0.99,0,0,0,0,0';
export GRT_MACRO_EXTENSION='0';
export LIB_TYPICAL='pdk/sky130A/libs.ref/sky130_fd_sc_hd/lib/sky130_fd_sc_hd__tt_025C_1v80.lib';
export MAX_FANOUT_CONSTRAINT='18';
export MAX_TRANSITION_CONSTRAINT='0.5';
export MERGED_LEF='./tmp/merged.nom.lef';
export OUTPUT_CAP_LOAD='33.442';
export PACKAGED_SCRIPT_0='openlane/scripts/openroad/gpl.tcl';
export PACKAGED_SCRIPT_1='openlane/scripts/openroad/common/io.tcl';
export PACKAGED_SCRIPT_2='openlane/scripts/openroad/common/set_rc.tcl';
export PACKAGED_SCRIPT_3='openlane/scripts/openroad/common/set_routing_layers.tcl';
export PACKAGED_SCRIPT_4='openlane/scripts/openroad/common/set_layer_adjustments.tcl';
export PACKAGED_SCRIPT_5='./tmp/floorplan/3-initial_fp.sdc';
export PACKAGED_SCRIPT_6='openlane/scripts/openroad/common/set_global_connections.tcl';
export PL_BASIC_PLACEMENT='0';
export PL_ROUTABILITY_DRIVEN='1';
export PL_SKIP_INITIAL_PLACEMENT='0';
export PL_TARGET_DENSITY='0.50';
export PL_TIME_DRIVEN='1';
export PL_WIRELENGTH_COEF='0.25';
export PROCESS_CORNER='nom';
export RT_CLOCK_MIN_LAYER='met3';
export RT_MAX_LAYER='met5';
export RT_MIN_LAYER='met1';
export SAVE_DEF='./tmp/placement/7-global.def';
export SAVE_NETLIST='./out.v';
export SAVE_ODB='./tmp/placement/7-global.odb';
export SAVE_POWERED_NETLIST='./tmp/placement/7-global.pnl.v';
export SCRIPTS_DIR='openlane/scripts';
export STA_PRE_CTS='1';
export STD_CELL_GROUND_PINS='VGND VNB';
export STD_CELL_POWER_PINS='VPWR VPB';
export TECH_METAL_LAYERS='li1 met1 met2 met3 met4 met5';
export VDD_NET='vccd1';
TOOL_BIN=${TOOL_BIN:-openroad}
$TOOL_BIN -exit $PACKAGED_SCRIPT_0
